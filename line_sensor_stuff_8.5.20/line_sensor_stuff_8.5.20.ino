...
LinesensorRight = analogRead(LinesensorPinRight); 
  ...
  error = rightAvgDistance -300;
}
for (int i = 0; i<=5; i++){
  
  LinesensorLeft = analogRead(LinesensorPinRight);
  sum = LinesensorLeft + sum;
  leftAvgDistance = sum/5;
  error = leftAvgDistance - 300;
}
...
if(leftAvgDistance<=0){
    motor1.SetSpeed((150*((abs(error)/40.0))*Kp));
    motor2.SetSpeed((200*((abs(error)/40.0))*Kp));
  }

  else if(leftAvgDistance>0){
  motor1.SetSpeed((-200*(1-(abs(error)/40.0)*Kp)));
  motor2.SetSpeed((-200*(1-(abs(error)/40.0)*Kp)));
  }

else if(rightAvgDistance>=0){
  motor1.SetSpeed((-200*(1-(abs(error)/40.0)*Kp)));
  motor2.SetSpeed((150*(1-(abs(error)/40.0)*Kp)));
  }
else if(rightAvgDistance>0){
  motor1.SetSpeed((-200*(1-(abs(error)/40.0)*Kp)));
  motor2.SetSpeed((-200*(1-(abs(error)/40.0)*Kp)));
 }
else if(((rightAvgDistance+leftAvgDistance)/2) > 1000){
